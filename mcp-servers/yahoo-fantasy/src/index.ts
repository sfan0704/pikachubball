...
let credentials: YahooCredentials | null = null;
...
async function refreshAccessToken(currentRefreshToken: string): Promise<{ accessToken: string; refreshToken: string; expiresIn: number }> {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    if (name === "set_credentials") {
      const typedArgs = args as unknown as YahooCredentials;
      credentials = {
    ...
    switch (name) {
      case "get_user_leagues": {
        const response = await fetch(
          "https://fantasysports.yahooapis.com/fantasy/v2/users;use_login=1/games;game_keys=nba/leagues?format=json",
          {
      ...
      case "get_league_standings": {
        const { leagueKey } = args as { leagueKey: string };
        const response = await fetch(
      ...
      case "get_team_roster": {
        const { teamKey } = args as { teamKey: string };
        const response = await fetch(
      ...
      case "get_league_scoreboard": {
        const { leagueKey, week } = args as { leagueKey: string; week?: number };
      ...
      case "get_player_stats": {
        const { playerKeys } = args as { playerKeys: string[] };
        const keysParam = playerKeys.join(",");
        const response = await fetch(
          `https://fantasysports.yahooapis.com/fantasy/v2/players;player_keys=${keysParam}/stats?format=json`,
          {
            headers: { Authorization: `Bearer ${accessToken}` },
          }
        );
      ...
      case "get_free_agents": {
        const { leagueKey, position, status, sort, count } = args as {
          leagueKey: string;
          position?: string;
          status?: string;
          sort?: string;
